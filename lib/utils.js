...
module.exports = {
  ...
  _getAbsolutePath: _getAbsolutePath,
  ...
  getSourcePaths: function(rootPath) {
    ...
    return paths.filter(p => fs.existsSync(_getAbsolutePath(p)));
  },
  ...
  getAddonPaths: function(rootPath, commandOptions, filterAddonsBy) {
    let pathPrefix = _getAbsolutePath(rootPath);
...
/**
   * Returns full path of desired project or folder
   *
   * If desired path begins with "/" or "\" then return original path
   * Otherwise prepend current working directory
   *
   * @param {object} commandOptions - arguments passed when script was executed
   */
function _getAbsolutePath(desiredPath) {
  if (path.isAbsolute(desiredPath)) {
    return desiredPath;
  } else {
    return path.join(process.cwd(), desiredPath);
  }
}
...
function _getEmberConfigFile(projectPath) {
  ...
  let emberConfigPath = _getAbsolutePath(emberConfigRelPath);
...
function _getEUCConfigFile(commandOptions) {
  ...
  const eucConfigPath = _getAbsolutePath(eucConfigRelPath);
